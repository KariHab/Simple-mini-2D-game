...
int	validate_if_map_is_playable(t_map *data)
{
	...
	if (data->number_of_player != 1)
		exit(ft_printf(RED "ERROR\nThis is a solo game.\n" WHITE));
	if (data->rectangle == 1)
		exit(ft_printf(RED "ERROR\nMap should be a clean rectangle.\n" WHITE));
	if (data->wall == 1)
		exit(ft_printf(RED "ERROR\nThis is not an open world map.\n" WHITE));
	if (data->number_of_teddy < 1)
		exit(ft_printf(RED "ERROR\nMust have one teddy in your room.\n" WHITE));
...
void	map_copy(t_map *data)
{
	...
	while (data->map[i])
	{
		data->flood.map_copy[i] = ft_calloc((data->column + 1), sizeof(char));
		if (!data->flood.map_copy[i])
		{
			free_all(data->flood.map_copy);
			exit(1);
		}
		ft_memcpy(data->flood.map_copy[i], data->map[i], data->column + 1);
		i++;
	}
}
