...
int	check_next_tile(t_map *data, char direction, char tile)
{
	...
	x = data->player.x_pos_player;
	y = data->player.y_pos_player;
	if ((direction == 'd' && data->map[y][x + 1] == tile)
		|| (direction == 'a' && data->map[y][x - 1] == tile)
		|| (direction == 's' && data->map[y + 1][x] == tile)
		|| (direction == 'w' && data->map[y - 1][x] == tile))
		return (0);
	else
		return (1);
}
...
void	collect_teddy(t_map *data, char direction)
{
	...
	x = data->player.x_pos_player;
	y = data->player.y_pos_player;
	if ((direction == 'd' && data->map[y][x + 1] == 'C')
		|| (direction == 'a' && data->map[y][x - 1] == 'C')
		|| (direction == 'w' && data->map[y - 1][x] == 'C')
		|| (direction == 's' && data->map[y + 1][x] == 'C'))
	{
		data->collected++;
		print_teddy(data);
	}
...
void	move_player(t_map *data, char direction)
{
	...
	if (direction == 'd')
		data->player.x_pos_player++;
	print_moves(data);
	if (data->can_exit == 1 && data->map[data->player.y_pos_player]
		[data->player.x_pos_player] == 'E')
		end_of_game(data);
